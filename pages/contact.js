import {
  ...
  const contact = () => {
      ...
      const onBlur = ({target}) => setFocused((prev) => ({...prev, [target.name]:true }))    
      const handleChange = ({ target }) =>
         ...
         const onSubmit = async ()=> {
          ...
          try{
              ...
              toast({
                  render: () => (
                    <Box color='white' textAlign={'center'} p={3} bg='blue.500'>
                      Message sent.
                    </Box>
                  ),          
                    status:'Success',
      ...
      <ChakraProvider className={s.container}>
        <Container className={`${s.contactCon} ${fontStyle.className}`}>
          ...
          <Container className={s.formCon}>
            ...
            <FormControl isRequired isInvalid={focused.email && !isValidEmail(values.email)}>
            ...
            <FormControl isRequired isInvalid={focused.phone && values.phone.length < 7}>
            ...
            <Button
             className={s.submitBtn}
              type="submit"
              disabled={!values.name || !values.email || !values.phone || !values.subject || !values.message}
              onClick={onSubmit}
              isLoading={isLoading}
             >Submit</Button>
          </Container>
