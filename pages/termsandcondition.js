import Head from "next/head";

const tc = () => {
  return (
    <>
      <Head>
        <title>QWERTY - Terms & Conditions</title>
      </Head>
      <div style={{ padding: "40px", color: "black" }}>
        <h1 style={{ textAlign: "center" }}>
          Terms and Conditions for QWERTY{" "}
        </h1>

        <h2>1. Introduction</h2>
        <p>
          Welcome to the QWERTY ("Store"). These terms and conditions ("Terms",
          "Agreement") are an agreement between the operator of QWERTY,
          ("QWERTY", "us", "we", or "our") and you ("User", "you" or "your").
          This Agreement sets forth the general terms and conditions of your use
          of the Store and any of its products or services (collectively,
          "Store" or "Services").
        </p>

        <h2>2. Agreement to Terms</h2>
        <p>
          By accessing or using the Store, you agree that you have read,
          understood, and agree to be bound by all of these Terms of Service. If
          you do not agree with any of these terms, you are prohibited from
          using or accessing this site. The materials contained in this Store
          are protected by applicable copyright and trademark law.
        </p>

        <h2>3. Use License</h2>
        <p>
          Permission is granted to temporarily download one copy of the
          materials (information or software) on QWERTY's online store for
          personal, non-commercial transitory viewing only. This is the grant of
          a license, not a transfer of title, and under this license, you may
          not: <br />
          a. modify or copy the materials; <br></br> b. use the materials for
          any commercial purpose, or for any public display (commercial or
          non-commercial); <br></br>c. attempt to decompile or reverse engineer
          any software contained on QWERTY's online store; <br></br>d. remove
          any copyright or other proprietary notations from the materials; or{" "}
          <br />
          e. transfer the materials to another person or "mirror" the materials
          on any other server. This license shall automatically terminate if you
          violate any of these restrictions and may be terminated by QWERTY at
          any time. Upon terminating your viewing of these materials or upon the
          termination of this license, you must destroy any downloaded materials
          in your possession whether in electronic or printed format.
        </p>
        <p>
          This license shall automatically terminate if you violate any of these
          restrictions and may be terminated by QWERTY at any time. Upon
          terminating your viewing of these materials or upon the termination of
          this license, you must destroy any downloaded materials in your
          possession whether in electronic or printed format.
        </p>

        <h2>4. Product Orders</h2>
        <p>
          By placing an order through the Store, you are making an offer to
          purchase the products or services you have selected. Once we receive
          your order, we will send you an email acknowledging receipt of your
          order and detailing the products or services you have ordered. This
          email is not an acceptance of your order, and any contract between us
          will not be formed until we send you an email to confirm that your
          order has been dispatched (the "Dispatch Confirmation").
        </p>
        <h2>5. Payment</h2>
        <p>
          Payment for products or services ordered through the Store can be made
          using a variety of methods as indicated on the Store at the time of
          purchase. You confirm that the payment method you are using is your
          own and that there are sufficient funds or credit available to cover
          the cost of the products or services.
        </p>
        <h2>6. Refunds and Returns</h2>
        <p>
          We want you to be completely satisfied with your purchase, but we
          understand that sometimes you may need to return an item. Please
          review our Return Policy for detailed information on our refund and
          return policy.
        </p>
        <h2>7. Disclaimer</h2>
        <p>
          The materials on QWERTY's website are provided on an 'as is' basis.
          QWERTY makes no warranties, expressed or implied, and hereby disclaims
          and negates all other warranties including, without limitation,
          implied warranties or conditions of merchantability, fitness for a
          particular purpose, or non-infringement of intellectual property or
          other violation of rights.
        </p>
        <h2>8. Limitations</h2>
        <p>
          In no event shall QWERTY or its suppliers be liable for any damages
          (including, without limitation, damages for loss of data or profit, or
          due to business interruption) arising out of the use or inability to
          use the materials on QWERTY's website, even if QWERTY or a QWERTY
          authorized representative has been
        </p>
        <br></br>
        <h2>Contact Us</h2>
        <p>
          If you have any questions about these Terms and Conditions, You can
          contact us:
        </p>
        <ul>
          <li>
            By email:
            <a style={{ fontWeight: "bold" }} href="mailto:info@qwerty.com">
              {" "}
              info@qwerty.com
            </a>
          </li>
        </ul>
      </div>
    </>
  );
};
export default tc;
